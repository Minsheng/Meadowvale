...
public class AdminUI extends javax.swing.JFrame {
    ...
    // When creating a new account, the following info will be updated:
    // 1. The first name and the last name of a person, where the full name cannot
    // be redundant in this application;
    // 2. The account type, either student or administrator / teacher
    // 3. The class a student belongs to if a new student account is to be created
    // 4. The user name of this new account will be generated with the last name
    // and first two letters of the first name of that person such that,
    // Lily Thompson will have a user name "thompsonli"
    // (However, this still DOES NOT eliminate user name redundancy
    // The user name generating method may be refactored later)
    private void newAccountButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_newAccountButtonActionPerformed
        ...
        // Display warning message if first name is blank
        if (firstName.length() == 0) {
            this.acctCreatedLabel.setText("Please enter a first name.");
            this.acctCreatedLabel.setForeground(Color.red);
            return;
        }
        
        // Display warning message if lirst name is blank
        if (lastName.length() == 0) {
            this.acctCreatedLabel.setText("Please enter a last name.");
            this.acctCreatedLabel.setForeground(Color.red);
            return;
        }
        
        // Create a username with last name and first two letters of first name
        String userName = lastName.toLowerCase()
                + firstName.substring(0, 2).toLowerCase();
        
        // Retrieve name using the mvApp entity class from the database
        // If such person already exists, display warning message
        ...
        String confirmMessage = new String();
        ...
        if (this.studentRadio.isSelected()) {
            if (this.addSSelectRecent.isSelected()) {
                ...
                int classNum = addAClassNum.getSelectedIndex() + 1;
                int classYear = addSSchoolYear.getYear();
                String classSeason = (String)addSSchoolSeason.getSelectedItem();
                String schoolTerm = classNum + " " + classSeason;
                String pickClass = "class_number = '" + classYear +
                        "' AND term = '" + schoolTerm + "'";
                List<MvClass> classes =
            ...
            String classNumStr = Integer.toString(mvClass.getClassNumber());
            String schoolTerm = mvClass.getTerm();

            confirmMessage = "Do you want to create the following user to" +
                    " class " + classNumStr  + ", " + schoolTerm +
                    "?\n" + "First Name: " + firstName + "\n" +
                    "Last Name: " + lastName + "\n" +
                    "User Type: Student";
        } else {
            confirmMessage = "Do you want to create the following user?\n" +
                    "First Name: " + firstName + "\n" +
                    "Last Name: " + lastName + "\n" +
                    "User Type: Admin";
        }
        ...
        int selection = JOptionPane.showConfirmDialog(null,
                confirmMessage, "Confirmation", 0);
        
        if (selection == 1) {
            return;
        }
        
        // Identify user type as 2: Student or 3: Admin
        ...
        // Retrieve the specified row in the db table MvPersonGroup
        String predicateGroupType = "id = " + groupType;
        List<MvPersonGroup> rl =
                mvApp.selectEntitiesBy(MvPersonGroup.class, predicateGroupType);
        ...
        // Insert the mvAddress into the db table
        ...
        // Insert the mvPerson into the db table
        ...
        // Create a new mvUser object with a default password
        ...
        if (this.studentRadio.isSelected()) {
            ...
            this.acctCreatedLabel.setText("Student account successfully created.");
            this.acctCreatedLabel.setForeground(Color.green);
        } else if (this.adminRadio.isSelected()) {
            this.acctCreatedLabel.setText("Admin account successfully created.");
            this.acctCreatedLabel.setForeground(Color.green);
        }
    ...
    // Select an active account and disable it
    private void disableAccountActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_disableAccountActionPerformed
        ...
        // Otherwise, identify account type and parse the name in the text field
        if (student == -1 && admin == -1) {
            ...
            if (student != -1) {
                ...
                if (fullName.equals("There is currently students enrolled in this class.")) {
                    this.acctDisabledLabel.setText("Please select a class.");
                    this.acctDisabledLabel.setForeground(Color.red);
                    String[] listData = {"Please select a class first."};
                    this.studentList.setListData(listData);
            ...
            String predicateName = " first_name = '" + firstName
                    + "' AND last_name = '" + lastName + "'";
            List<MvPerson> rl = mvApp.selectEntitiesBy(MvPerson.class, predicateName);
            ...
            String predicateID = " mv_person_id = " + ID;
            List<MvUser> r2 = mvApp.selectEntitiesBy(MvUser.class, predicateID);
    ...
    // Hide the current interface and make a new Log-in UI visible to the user
    ...
    // When creating a class, the following info will be filled out or selected
    // by the user and the corresponding database entries will be updated
    // after the "Create Class" button is pressed:
    // Class number, class name, school term, season, and required co-op hours
    private void create_classActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_create_classActionPerformed
        int classNumber = classNum.getSelectedIndex() + 1;
        int year = classYear.getYear();
        String season = (String)classSeason.getSelectedItem();
        String term = year + " " + season;

        String coopHours = reqCoopHrs.getText();
        String className = this.classNameField.getText();

        if (className.equals("")) {
            this.classCreatedLabel.setText("Please enter a class name.");
            this.classCreatedLabel.setForeground(Color.red);
            return;
        }

        if (coopHours.length() == 0) {
            this.classCreatedLabel.setText("Please enter a positive value for co-op hour.");
            this.classCreatedLabel.setForeground(Color.red);
        } else {
            try {
                int coopHoursValue = Integer.parseInt(coopHours);
                if (coopHoursValue <= 0) {
                    this.classCreatedLabel.setText("Please enter a positive value for co-op hour.");
                    this.classCreatedLabel.setForeground(Color.red);
                } else {
                    // Try to retrieve a class using predicateOfClass from the database
                    // If no class with similar class number and term is found,
                    // A new class is safe to be created
                    String predicateOfClass = "class_number = '" + classNumber +
                            "' AND term = '" + term + "'";
                    List<MvClass> classesList =
                            mvApp.selectEntitiesBy(MvClass.class, predicateOfClass);
                    if (classesList.isEmpty()) {
                        String confirmMessage = "Do you want to create the following class?\n" +
                                "Class Number: " + classNumber + "\n" +
                                "School Term: " + term + "\n" +
                                "Required Co-op Hours: " + coopHoursValue;
                        int selection = JOptionPane.showConfirmDialog(null,
                                confirmMessage, "Confirmation", 0);
                        if (selection == 0) {
                            MvClass mvClass = new MvClass();
                            mvClass.setClassName(className);
                            mvClass.setClassNumber(classNumber);
                            mvClass.setTerm(term);
                            mvClass.setRequiredCoopHours(coopHoursValue);
                            mvApp.MvClassController.create(mvClass);
                            this.classCreatedLabel.setText("Class successfully created.");
                            this.classCreatedLabel.setForeground(Color.green);
                        } 
                    } else {
                            this.classCreatedLabel.setText("Such class already exists.");
                            this.classCreatedLabel.setForeground(Color.red);
                    }
                }
            } catch (Exception ex) {
                this.classCreatedLabel.setText("Please enter a positive co-op hour.");
                this.classCreatedLabel.setForeground(Color.red);
            }
        }
        prepareAccountTab();
    ...
    // Take a date object which is to be returned by picking from the jCalendar
    // Return a string in the format YY-MM-DD from a Date object
    ...
    // Add co-op weeks to either:
    //     1) An existing class created recently or
    //     2) A new class with class number, school term, and season
    // All info associated with the co-op weeks are:
    // Week number, week start date, week end date, and due date
    // Additionally, description associated with each day 
    // such as "Chapel" or "Holiday" in a particular week may be added by a teacher
    private void addWeekActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addWeekActionPerformed
        MvClass mvClass = null;

        // Select from a most recently creaated class and add weeks to it
        ...
        // Create a new class and add weeks to it
        } else if (this.selectManual.isSelected()) {
            int classNumber = selectClassNum.getSelectedIndex() + 1;
            int year = selectSchoolYear.getYear();
            String season = (String)selectSchoolSeason.getSelectedItem();
            String term = year + " " + season;
            String predicateOfClass = "class_number = '" + classNumber +
                            "' AND term = '" + term + "'";
            List<MvClass> classes =
        ...
        String weekValue = Integer.toString(weekNum.getSelectedIndex() + 1);
        Date startDate = weekStart.getDate();
        Date endDate = weekEnd.getDate();
        Date dueDate = weekDueDate.getDate();

        // All types of dates are required to be specified
        ...
        String startDateStr = convertDate(startDate);
        String endDateStr = convertDate(endDate);
        String dueDateStr = convertDate(dueDate);

        int classID =  mvClass.getId();
        String predicateStartDate = "first_day = '" + startDateStr +
                "' AND class_id = " + classID;
        String predicateEndDate = "last_day = '" + endDateStr +
                "' AND class_id = " + classID;
        String predicateDueDate =  "due_date = '" + dueDateStr +
                "' AND class_id = " + classID;
        String predicateWeek = "class_id = " + classID +
                " AND week_number = '" + weekValue + "'";

        List<MvWeek> weekList1 = mvApp.selectEntitiesBy(MvWeek.class, predicateStartDate);
        List<MvWeek> weekList2 = mvApp.selectEntitiesBy(MvWeek.class, predicateEndDate);
        List<MvWeek> weekList3 = mvApp.selectEntitiesBy(MvWeek.class, predicateWeek);
        List<MvWeek> weekList5 = mvApp.selectEntitiesBy(MvWeek.class, predicateDueDate);
        ...
        if (!(weekList1.isEmpty())) {
            ...
            String predicateMaxWeek = "class_id = " + classID +
                " AND week_number = (select max(week_number) from mv_week " +
                "where class_id = " + classID + ")";
            List<MvWeek> weekList4 = mvApp.selectEntitiesBy(MvWeek.class, predicateMaxWeek);
            ...
            String confirmMessage = "Do you want to add the following week to" +
                                " class " + classNumber  + ", " + term +
            ...
            // After OK is selected, all corresponding info will be set
            // in the databse entries and a successfuly message will be shown
            if (selection == 0) {
                MvWeek mvWeek = new MvWeek();
                mvWeek.setFirstDay(startDate);
                mvWeek.setLastDay(endDate);
                mvWeek.setClassId(mvClass);
                mvWeek.setWeekNumber(Integer.parseInt(weekValue));
    ...
    // Exit safely from the appliction
    private void exitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_exitActionPerformed
        String confirmMsg = "Are you sure you want to exit?";
        int selection = JOptionPane.showConfirmDialog(null,
                confirmMsg, "Confirmation", 0);
        if (selection == 1) {
            return;
        }
        System.exit(1);
    ...
    // Select an inactive account and enable it
    private void enableAccountActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_enableAccountActionPerformed
        ...
        if (student == -1 && admin == -1) {
            ...
            // Otherwise, get the account type and parse the names in the text field
            String fullName = new String();
            if (student != -1) {
                fullName = (String) this.studentList.getSelectedValue();
                if (fullName.equals("Please select a class first.")) {
            ...
            // Retrieve person info using predicatePerson from entity class MvPerson
            String predicatePerson = " first_name = '" + firstName
                    + "' AND last_name = '" + lastName + "'";
            List<MvPerson> personList =
                    mvApp.selectEntitiesBy(MvPerson.class, predicatePerson);
            MvPerson mvPerson = personList.get(0);

            // Get the selected person's id
            int ID = mvPerson.getId();

            // Enable the selected person
...
//                System.out.println("------");
    ...
    private void updateStudentList() {
        // Retrieve classID using predicateClass from entity class MvClass
        String predicateClass = "class_number = " + this.curClassNumber +
                " AND term = '" + this.curClassTerm + "'";
        List<MvClass> classesList = mvApp.selectEntitiesBy(MvClass.class, predicateClass);
        MvClass mvClass = classesList.get(0);
        int classID = mvClass.getId();

        String selectStudent = "person_group_id = 2";
        List<MvPerson> personList1 = mvApp.selectEntitiesBy(MvPerson.class, selectStudent);
    ...
    private void studentListMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_studentListMouseClicked
        ...
        // Otherwise, get the account type and parse the names in the textfield
        String fullName = new String();
        if (student != -1) {
            fullName = (String) this.studentList.getSelectedValue();
            if (fullName.equals("Please select a class first.")) {
